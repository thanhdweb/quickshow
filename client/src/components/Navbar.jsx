  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        ...
        <Link onClick={() => { scrollTo(0, 0), setIsOpen(false) }} to={'/'} className='mx-4 text-lg font-medium hover:text-primary transition'>Theaters</Link>
        <Link onClick={() => { scrollTo(0, 0), setIsOpen(false) }} to={'/'} className='mx-4 text-lg font-medium hover:text-primary transition'>Releases</Link>
        {
      ...
      <div className='flex items-center gap-8'>
        <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointe' />
        {
          !user ? (
